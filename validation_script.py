...
def validate_model(session, val_data, eye_left, eye_right, face, face_mask, predict_op):
  ...
  for i in range(0, int(num_iters)):
    ...
    eye_left_batch = val_eye_left[start_index:end_index, :]/255.0
    eye_right_batch = val_eye_right[start_index:end_index, :]/255.0
    face_batch = val_face[start_index:end_index, :]/255.0
    face_mask_batch = val_face_mask[start_index:end_index, :]/255.0
    y_batch = val_y[start_index:end_index, :]

    eye_left_batch = eye_left_batch - eye_left_batch.mean()
    eye_right_batch = eye_right_batch - eye_right_batch.mean()
    face_batch = face_batch - face_batch.mean()
    face_mask_batch = face_mask_batch - face_mask_batch.mean()
    ...
    yp = session.run(predict_op, 
                     feed_dict={eye_left: eye_left_batch, 
